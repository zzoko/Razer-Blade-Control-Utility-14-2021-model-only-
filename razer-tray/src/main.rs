...
impl ProgramState {
    ...
    fn create_menu_and_handlers(
        ...
        */
        ...
        .filter(|boost| *boost != CpuBoost::Overclock) // <-- hide Overclock globally
            .map(|boost| {
...
/* replaced with one below to skip Silent mode
...
fn get_next_perf_mode(&self) -> DeviceState {
    DeviceState {
        perf_mode: match self.device_state.perf_mode {
            // If currently Balanced -> go to Custom
            PerfMode::Balanced(..) => {
                PerfMode::Custom(CpuBoost::Boost, GpuBoost::High, MaxFanSpeedMode::Disable)
            }
            // If currently Custom -> go to Balanced(Auto)
            PerfMode::Custom(..) => PerfMode::Balanced(FanSpeed::Auto),
            // Any other/unknown state -> default to Balanced(Auto)
            _ => PerfMode::Balanced(FanSpeed::Auto),
        },
        ..self.device_state
    }
}
